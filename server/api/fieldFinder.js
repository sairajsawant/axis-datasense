...
function findFields(coll, currentKey, first, fields, onlyObjects) {
  ...
  if (determineType.determineType(coll) === "array" && onlyObjects) {
    return newFields;
  }
  ...
  Object.keys(coll).forEach((field) => {
    ...
    if (currentKey) {
      ...
      const fieldType = determineType.determineType(coll[field]);
...
function init(collection, checkObjects, onlyObjects) {
  let fields = [];
    ...
    Object.keys(collection).forEach((field) => {
      if (collection[field] instanceof Array) {
        explorationSet.push({
          ...
          type: determineType.determineType(collection[field]),
        });
      }
    });
  }
...
module.exports = {init, findFields}
